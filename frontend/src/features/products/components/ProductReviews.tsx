...
const ReviewForm = ({ productId, onSuccess }: { productId: number; onSuccess?: () => void }) => {
  ...
  <div className='bg-linear-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100'>
    <div className='flex items-start gap-4'>
      ...
      <div className='flex-1'>
        <h3 className='font-semibold text-gray-900 mb-1'>Đăng nhập để đánh giá sản phẩm</h3>
      ...
      <div className='space-y-4'>
        {comments.length === 0 ? (
          ...
          comments.map((comment) => (
            <CommentItem key={comment.id} comment={comment} onReply={(id) => setReplyingTo(id)} />
          ))
        )}
      </div>
    </div>
      ...
      <div className='flex items-center gap-6 border-b border-gray-200 mb-6'>
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors relative ${
            activeTab === 'reviews' ? 'text-[#1E3A8A]' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors relative ${
            activeTab === 'comments' ? 'text-[#1E3A8A]' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
        ...
        <>
          ...
          <div className='bg-indigo-50/50 border border-indigo-100 p-6 rounded-xl flex flex-col md:flex-row items-center gap-8 mb-8'>
            <div className='text-center items-center md:border-r border-indigo-200 md:pr-8'>
              ...
              <p className='text-sm text-gray-500 mt-2'>{totalRatings.toLocaleString()} đánh giá</p>
            </div>
