...
const login = async (req, res) => {
  try {
    ...
    if (!user.password)
      return errorResponse(res, "Tài khoản này đăng nhập bằng Google", 400);
...
const refreshToken = async (req, res) => {
  try {
    ...
    if (savedToken.token !== token) {
      ...
      return errorResponse(
        res,
        "Phát hiện bất thường, vui lòng đăng nhập lại",
        401,
      );
    }
    ...
    // Bước 6: Xóa token cũ, lưu token mới (Rotate)
    await tokenService.deleteRefreshToken(userId, tokenId);
    await tokenService.saveRefreshToken(userId, newTokenId, newRefreshToken);
